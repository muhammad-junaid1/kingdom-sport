      ...
      {(!isMobile) && 
        ...
        <div className="navbar__main-items-container">
          ...
          {cryptoData.map((item, index) => {
            return <NavbarItem isDropDown={false} Icon={SVGIcons.cryptoIcons[item.icon]} text={item.text} number={item.number} collapse={collapse} activeRadio={checked === "all" ? false : (checked === index)} onCheck={() => checked === index ? setChecked(index) : setChecked(index)} isCryptoItem={true} target={`/crypto/${index+1}`}/>
          })}
          </>
          }
